...
**Off-call exclusion (2026-09):** A member who is "Off-Call" in D4H is not paged by a group callout. At Step 0.7 of `/send-notification` — before the Firestore skeleton and every side effect — `_build_off_call_plan` reads `d4h.get_off_call_now` (`duties?type=OFF` overlapping now, the UI's "Off-Call Now" panel) and expands each selected group with `list_group_member_contacts`, joining D4H `member.ref` to the 3-digit OCEAN# parsed from the EB contact's `externalId`; a member whose Ref is not a 3-digit OCEAN# cannot be matched and is still paged — the Event Log names them and asks for the Ref to be fixed. A live send then goes out as `contactIds` = group members minus off-call, plus anyone the dispatcher picked by name; `groupIds` is kept only for a group whose expansion failed. Groups are expanded rather than trimmed because Everbridge accepts and stores `excludedContactIds` but ignores it on a group send (live-tested). Draft mode keeps the groups (a hand-send from the EB UI drops `contactIds`) and the Event Log line reads `Off-call in D4H: <names> — still in the Everbridge draft; remove them in Everbridge before sending`. Every step fails open: a D4H outage or a group-fetch failure degrades to the plain group send. The result surfaces as Event Log lines (dispatcher textarea and the D4H incident description), the `🚫 Unavailable in D4H (not paged)` line of the `#active-incidents` tally, and counts-only Cloud Run logs. A dispatcher override ("page everyone anyway") is a separate change, not yet shipped.
...
**Off-call read (`get_off_call_now`):** `/send-notification` also reads `GET /v3/team/{teamId}/duties?type=OFF` for the periods overlapping now (see Off-call exclusion under Everbridge); the parser keeps only `member.id`/`ref`/`name` and never reads a duty's `notes`, which may hold medical or family reasons.
