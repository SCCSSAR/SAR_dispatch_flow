...
│  [Primary dispatch — click in workflow order: Map → EB → WhatsApp]: │
...
│     WhatsApp                 (wa.me deep link — permanent manual)   │
│                                                                     │
...
| WhatsApp | Permanent manual-fill | `wa.me` deep link, pre-filled SOP template; STAGING autofills from first recommendation |
...
**Rollout state:** `SLACK_MODE=full` on SCCSSAR dev (as of Phase 1.9, July 2026) — real invites sent to all confirmed YES responders; Slack is the official incident coordination channel. The `_partition_arrivals_for_shadow()` 3-way classification helper is retained in the codebase for any future shadow-mode caller but is not invoked in full mode. Personal dev remains `SLACK_MODE=shadow` (hardcoded per CLAUDE.md Locked Decision). WhatsApp `wa.me` deep link is still supported as a manual fallback.
...
17. **Dispatch** — Dispatcher uses the action buttons (in workflow order — Map first so the Slack welcome message includes the CalTopo URL):
    ...
    - **WhatsApp** — opens `wa.me` with pre-filled SOP template; STAGING autofills from first recommendation; OCEAN# auto-filled from EB dispatcher record. (Manual fallback; Slack is now the official incident coordination channel.)
...
2. **Dispatcher textarea** — The Koester analysis is included in the **Full Incident Summary** section (the bottom block of the two-section textarea layout). Search managers use it for planning; it is not part of the WhatsApp pre-fill or the Slack welcome message.
...
*Last updated: July 2026 — Phase 1.9. Per-change history lives in `git log` + [CHANGELOG.md](../CHANGELOG.md).*
