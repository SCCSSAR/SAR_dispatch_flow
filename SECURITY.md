...
`filter`, `bias`, `x`, `y`, `latitude` and `longitude` parameters while leaving host and path intact; it is pinned against the
...
| **US Census Geocoder** | The LKP coordinate **rounded to 3 dp (~110 m)** plus 8 derived points 1 km out, to read the 2020 census block's urban/rural flag for the LPB environment line. No identifier attached. Every dispatch with a geocoded LKP | Anonymous (keyless) | Per the Census Bureau's public terms; no account |
| **Open-Meteo Elevation API** | 25 points within 2 km of the LKP, **rounded to 3 dp**, for terrain relief. **Only when the LKP is classified rural**, which includes an urban block surrounded by wilderness (the edge rule); an urban LKP in an urban area never makes this call | Anonymous (keyless) | Per Open-Meteo's public terms; no account |
...
| 2026-09-10 | pre-merge, branch `feat/lpb-environment-classifier` | Re-review trigger: **two new third-party integrations** for the LPB environment line, the US Census Geocoder (2020 block urban/rural flag) and Open-Meteo (elevation). Both keyless, HTTPS, TLS verification and no-redirect defaults left intact. **New data flow:** the LKP, rounded to 3 dp (~110 m) before leaving, goes to the Census Bureau on every dispatch and to Open-Meteo on rural dispatches only — less than Nominatim, Google and Geoapify already receive. `x`, `y`, `latitude`, `longitude` added to the root-handler redaction (`_PII_QS_RE`), pinned by a test on both providers' request shapes. Response content cannot inject: only the exact `U`/`R` flag and a numeric relief are used, and the frontend writes `/ocr` text into a textarea's `.value`. 0 HIGH / 0 MEDIUM / 0 LOW. |
